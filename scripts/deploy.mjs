 ...
 *
 *   { "vault": "/path/to/vault" }
 *   { "vaults": ["/path/one", "/path/two"] }
 *
 * Fails loudly on a missing target or stale build rather than silently
...
// means testing code that was never built. Refuse rather than report success.
if (statSync("main.js").mtimeMs < newestMtime("src")) {
    throw new Error(
        "main.js is older than files in src/. Run 'npm run build' (or 'npm run build:deploy')."
    );
}
